import React from 'react';
import './style.css';

const Home = () => {
  return (
    <div className="desktop">
      <div className="overlap-wrapper">
        <div className="overlap">
          <div className="home-page">
            <div className="div">
              <div className="navbar">
                <p className="docu-safe">
                  <span className="text-wrapper">Docu </span>
                  <span className="span">Safe</span>
                </p>
                <div className="text-wrapper-2">Documentation</div>
                <div className="text-wrapper-3">Community</div>
                <div className="text-wrapper-4">Blog</div>
                <div className="text-wrapper-5">Donate</div>
                <div className="text-wrapper-6">Search</div>
                <img className="search" alt="Search" src="search.png" />
              </div>
              <div className="div">
                <div className="overlap-group">
                  <p className="p">
                    <span className="text-wrapper">Docu </span>
                    <span className="span">Safe</span>
                  </p>
                  <p className="is-a-powerful-free">
                    is a powerful free, open <br />
                    source tool for working <br />
                    with messy data
                  </p>
                  <div className="overlap-group-wrapper">
                    <div className="overlap-group-2">
                      <img className="group" alt="Group" src="group.png" />
                      <div className="div-wrapper">
                        <div className="overlap-group-2">
                          <div className="group-2" />
                          <img className="img" alt="Group" src="image.png" />
                        </div>
                      </div>
                      <img className="group-3" alt="Group" src="group-2.png" />
                      <img className="group-4" alt="Group" src="group-3.png" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="main-features">
              <div className="overlap-2">
                <div className="rectangle" />
                <div className="text-wrapper-7">Data Hygiene Solutions</div>
              </div>
              <div className="element-img-wrapper">
                <img
                  className="element-img"
                  alt="Element img"
                  src="4-img.png"
                />
              </div>
              <div className="overlap-3">
                <div className="element-img-2">
                  <div className="rectangle-2" />
                  <div className="overlap-group-3">
                    <img className="group-5" alt="Group" src="group-4.png" />
                    <img className="group-6" alt="Group" src="group-5.png" />
                    <img className="vector" alt="Vector" src="vector.svg" />
                    <img className="vector-2" alt="Vector" src="image.svg" />
                    <img className="vector-3" alt="Vector" src="vector-2.svg" />
                    <img className="vector-4" alt="Vector" src="vector-3.svg" />
                    <img className="vector-5" alt="Vector" src="vector-4.svg" />
                    <img className="group-7" alt="Group" src="group-6.png" />
                  </div>
                </div>
                <div className="data-highlight">
                  Data Highlight &amp; Hidden
                </div>
              </div>
              <div className="overlap-4">
                <div className="text-wrapper-8">Data Encryption</div>
              </div>
              <div className="img-wrapper">
                <img
                  className="element-img-3"
                  alt="Element img"
                  src="2-img.png"
                />
              </div>
              <div className="data-classification">
                <div className="overlap-5">
                  <div className="text-wrapper-9">Data Classification</div>
                </div>
                <div className="overlap-group-3">
                  <div className="element-img-4">
                    <div className="overlap-6">
                      <img
                        className="vector-6"
                        alt="Vector"
                        src="vector-5.svg"
                      />
                      <div className="group-8">
                        <div className="overlap-group-4">
                          <img
                            className="vector-7"
                            alt="Vector"
                            src="vector-6.svg"
                          />
                          <div className="group-9" />
                        </div>
                      </div>
                      <div className="group-10" />
                      <img className="group-11" alt="Group" src="group-7.png" />
                    </div>
                  </div>
                </div>
              </div>
              <div className="text-wrapper-10">Main Features</div>
              <div className="rectangle-3" />
            </div>
          </div>
          <footer className="footer">
            <div className="overlap-7">
              <div className="text-wrapper-11">Get OpenRefine</div>
              <a
                className="text-wrapper-12"
                href="https://openrefine.org/download"
                rel="noopener noreferrer"
                target="_blank"
              >
                Download
              </a>
              <div className="text-wrapper-13">User manual</div>
              <div className="text-wrapper-14">Get involved</div>
              <div className="text-wrapper-15">Extensions</div>
              <div className="text-wrapper-16">External resources</div>
              <div className="text-wrapper-17">Gitter chat</div>
              <div className="text-wrapper-18">GitHub repository</div>
              <div className="what-s-new">What&#39;s new</div>
              <div className="text-wrapper-19">Contributing</div>
              <div className="text-wrapper-20">Forum</div>
              <div className="text-wrapper-21">Other distributions</div>
              <div className="text-wrapper-22">Privacy notice</div>
              <div className="text-wrapper-23">Twitter</div>
              <div className="text-wrapper-24">Documentation</div>
              <div className="text-wrapper-25">Community</div>
            </div>
          </footer>
        </div>
      </div>
    </div>
  );
};

export default Home;
